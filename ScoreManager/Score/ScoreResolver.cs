...
namespace net.tkcore
{
    ...
    [Resolver(Author = "TKCore", CreateDate = "2020-10-27",
        Description = "Score(Score)的数据访问层类")]
    public class ScoreResolver : Tk5TableResolver
    {
        internal const string DATAXML = "Score/Score.xml";
        /// <summary>
        /// 建构函数，设置附着的Xml文件。
        /// </summary>
        /// <param name="context">数据库连接上下文</param>
        /// <param name="source">附着的数据源</param>
        public ScoreResolver(IDbDataSource source)
            : base(DATAXML, source)
        {
            AutoUpdateKey = true;
            AutoTrackField = true;
