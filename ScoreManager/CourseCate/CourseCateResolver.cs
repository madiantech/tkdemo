...
{
    /// <summary>
    ///  CourseCate(CourseCate)的数据访问层类
    /// </summary>
    [Resolver(Author = "TKCore", CreateDate = "2020-10-27",
        Description = "CourseCate(CourseCate)的数据访问层类")]
    public class CourseCateResolver : Tk5TreeTableResolver
    {
        internal const string DATAXML = "CourseCate/CourseCate.xml";
        /// <summary>
        /// 建构函数，设置附着的Xml文件。
        /// </summary>
        /// <param name="context">数据库连接上下文</param>
        /// <param name="source">附着的数据源</param>
        public CourseCateResolver(IDbDataSource source)
            : base(DATAXML, source)
        {
            AutoUpdateKey = true;
            AutoTrackField = true;
